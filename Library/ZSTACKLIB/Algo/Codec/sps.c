...
void sps_dump(struct sps *sps)
{
    printf("seq_parameter_set_data( ) {\n");
    printf(INDENT1_FORMAT, "profile_idc", sps->profile_idc);
    printf(INDENT1_FORMAT, "level_idc", sps->level_idc);
    printf(INDENT1_FORMAT, "seq_parameter_set_id", sps->seq_parameter_set_id);
	...
	if (sps->profile_idc == 100 || sps->profile_idc == 110 ||
        ...
        printf("  if( profile_idc = = 100 || profile_idc = = 110 || ...\n");
        printf(INDENT2_FORMAT, "chroma_format_idc", sps->chroma_format_idc);
    }
}

struct sps *sps_proc(unsigned char *buffer, unsigned int length)
{
	int i;
