...
int avmips_get_ves_desc(struct ringbuffer *r, unsigned int channel)
{
    int ret;
    ...
    unsigned int prev_wp = 0, prev_rp = 0;
    u64 prev_ms = 0, cur_ms = 0;
    unsigned int speed_wp, speed_rp;
    
    ret = dbg_avmips_read32(GET_VIDEO_DEBUG_INFO_VESDESCADDR | (channel<<6), &pts_addr, 1);
    if (0 != ret) {
    ...
    if(((pts_addr & SX6_HW_DEMUX_REG_BASE_MASK) == SX6_HW_DEMUX_REG_BASE)
        || ((pts_addr & UXL_HW_DEMUX_REG_BASE_MASK) == UXL_HW_DEMUX_REG_BASE)
        || ((pts_addr & 0xFFFF0000) == 0x10010000))
    {
    ...
    if (flag_hardware_demux) {
        ...
        dbg_host_read8(pts_addr, (unsigned char *)&HWDesc, sizeof(struct HWDemuxVESDesc_t));
        ...
        dbg_host_read8(pts_addr, (unsigned char *)&AVDesc, sizeof(struct AVStreamVESDesc_t));
    ...
    if (prev_wp < r->wp) {
        speed_wp = (r->wp - prev_wp) / (u32)(cur_ms - prev_ms);
    }
    else {
        speed_wp = (r->wp - r->start + r->end - prev_wp) / (u32)(cur_ms - prev_ms);
    }

    if (prev_rp < r->rp) {
        speed_rp = (r->rp - prev_rp) / (u32)(cur_ms - prev_ms);
    }
    else {
        speed_rp = (r->rp - r->start + r->end - prev_rp) / (u32)(cur_ms - prev_ms);
    }
