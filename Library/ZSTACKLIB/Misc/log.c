...
static int add_to_log_mask(char *name)
{
    ...
    // find a empty slot
    ret = module_match(name);
    if (0 != ret) {
        strcpy(log_modules[ret].name, name);
        log_modules[ret].init = 1;
    }
...
int _log(const char * name, int level, const char *fmt, ...)
{
    char _prefix[128] = {0};
    ...
    // Check the message
    if ((level > 0) && (0 != module_match(name))) {
        printf("not match\n");
        return 0;
    }
