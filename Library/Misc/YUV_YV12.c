...
static unsigned short GetU(unsigned char* buffer, int width, int height, int x, int y)
{
	...
	value = buffer[width * height + (width * height / 4) + (width / 2) * (y / 2) + x / 2];
...
static unsigned short GetV(unsigned char* buffer, int width, int height, int x, int y)
{
	...
	value = buffer[width * height + (width / 2) * (y / 2) + x / 2];
...
{
	buffer[width * height + (width * height / 4) + (width / 2) * (y / 2) + x / 2] = u;

}

static void SetV(unsigned char* buffer, int width, int height, int x, int y, unsigned short v)
{
	buffer[width * height + (width / 2) * (y / 2) + x / 2] = v;

}

struct YUV_Wrapper yuv_wrapper_yv12 = {
	FrameSize,
	GetY, GetU, GetV,
	SetY, SetU, SetV
};
