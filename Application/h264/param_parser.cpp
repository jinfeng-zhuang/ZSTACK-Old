...
int param_parser(int argc, char *argv[], struct application *app)
{
    ...
    if (0 == app->param.output_filename[0]) {
#ifdef _WIN32
        GetLocalTime(&time);
        _snprintf(app->param.output_filename, FILENAME_MAX, "h264_%d-%02d-%02d_%02d-%02d-%02d.log",
            time.wYear,
            time.wMonth,
            time.wDay,
            time.wHour,
        ...
        snprintf(app->param.output_filename, FILENAME_MAX, "h264_%d-%02d-%02d_%02d-%02d-%02d.log",
            tm->tm_year + 1900,
            tm->tm_mon + 1,
            tm->tm_day,
            tm->tm_hour,
